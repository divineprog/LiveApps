...
// Default base directory. Must NOT end with a slash.
var BasePath = '/HackathonDemos/BasicWebUI/LocalFiles'
...
{
  var file = GetFileStatus(BasePath + path)
  if (!file)
  {
  ...
  if (file.isDirectory())
  {
    ...
    var indexFile = GetFileStatus(BasePath + path)
    if (!indexFile)
    {
      FileNotFoundResponse(path, response)
      return
  ...
  WriteFileResponse(BasePath + path, response)
}
...
// Main handler for command requests.
function HandleCommandRequest(request, response)
{
  var command = unescape(URL.parse(request.url).pathname)
  
  console.log('Command: ' + command)
  
  if (0 == command.indexOf('/SetBasePath'))
  {
    BasePath = command.substr(13)
    console.log('BasePath: ' + BasePath)
  }
